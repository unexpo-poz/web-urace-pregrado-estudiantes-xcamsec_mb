...
function prepdata(fp,fd) {
    ...
    with(fp) {
        ...
        while(i < (CB.length - 1)){
		  cod_uc = CB[i].value.split(" ");
		  if (cod_uc[5] !='0'){
/* $_POST['asignaturas'] lleva :

CODIGO1 SECCION1 condREP1 COLOR1 
CODIGO2 SECCION2 condREP2 COLOR2
CODIGO3 SECCION3 condREP3 COLOR3
...    
CODIGON SECCIONN condREPN COLORN

Donde:
CODIGO = cod_uc[0]; 
SECCION = cod_uc[5] (Podria concatenarsele el grupo de laboratorio);
condREP = cod_uc[6];
COLOR = cod_uc[7];
*/
			...
			//if (document.getElementById('g'+cod_uc[0]).value) {
			...
			//alert(document.getElementById('sincup'+cod_uc[0]).value)

			scola = document.getElementById('sincup'+cod_uc[0]).value;

			scod_uc += " " + cod_uc[6] + " " + cod_uc[7] + " " + scola;
			//alert(scod_uc);
...
//          if (CB[j].selectedIndex != '0'){ 
	   ...
	   while(y <= i){
		...
		val = cola[y].value.split(" ");               
        c_asigna   = val[0]; // Estatus de la asignatura (G,B,Y,X);
		//alert(c_asigna);
		  if (document.getElementById('sincup'+c_asigna).selectedIndex != 0){
    ...
    for(i=0;i<7;i++){
        ...
        switch (cod_uc[7]) { // de acuerdo a la seleccion y estatus, se establece el color:
            ...
            case 'B' :  lcolor='#99CCFF'; //azul : INSCRITO
						...
						c_asigna = codigo[0];
						seccion = codigo[5];
						//alert('0: '+codigo[0]+' 1: '+codigo[1]+' 2: '+codigo[2]+' 3: '+codigo[3]+' 4: '+codigo[4]+' 5: '+codigo[5]);
						if ((document.getElementById(c_asigna+'horas_lab').value > 0) && (document.getElementById(c_asigna+'horas_teo').value > 0) && (document.getElementById(c_asigna+'seccionI').value != seccion)){
							if (seccion == 0) {
								seccion = document.getElementById(c_asigna+'grupo').value;
								//busca_grupo(c_asigna,seccion);
							}
							//alert(seccion);
							//seccion = (seccion == "") ? codigo[5] : seccion;
							busca_grupo(c_asigna,seccion);
						...
						//si selecciona la misma que tiene inscrita
						if ((document.getElementById(c_asigna+'horas_lab').value > 0) && (document.getElementById(c_asigna+'horas_teo').value > 0) && (document.getElementById(c_asigna+'seccionI').value == seccion)){
							//document.getElementById('g'+c_asigna).selectedIndex = 0;
							seccion = document.getElementById(c_asigna+'grupo').value;
							busca_grupo(c_asigna,seccion);
...
function Inscribirme() {
    ...
    if ((document.totales.t_mat.value != "0" || tieneRetiradas())) {
		...
		for (i=0;i < c_asigna.length-1; i=i+5){
			color = c_asigna[i+3];
			seccion = c_asigna[i+1].substring(0,2);
			if ((document.getElementById(c_asigna[i]+'horas_lab').value > 0) &&
				(document.getElementById(c_asigna[i]+'horas_teo').value > 0) && 
				(document.getElementById('g'+c_asigna[i]).selectedIndex == 0) &&
				(document.getElementById(c_asigna[i]+'inscrita').value == 1) &&
				(color == 'B') && 
				(document.getElementById(c_asigna[i]+'seccionI').value != seccion)
			){
				materias+= c_asigna[i]+", ";
				error++;
			}
...
function checkCols() {
	with(document.pensum) {
		...
		while(j < k){
			...
			c_asigna   = val[0];
			if (document.getElementById('sincup'+c_asigna).selectedIndex != 0){
				document.getElementById('concup'+c_asigna).disabled = true;
			}else{
				document.getElementById('concup'+c_asigna).disabled = false;
			}

			origen = document.getElementById('ins'+c_asigna).value;
			dest = document.getElementById('concup'+c_asigna).value.split(" ");
			destino=dest[5];

			if (destino != origen){
				document.getElementById('sincup'+c_asigna).disabled = true;
			}else{
				if (document.getElementById('sincup'+c_asigna).value != '-'){
					document.getElementById('sincup'+c_asigna).disabled = false;
				}
			}
/// MEJORAR /// MEJORAR 
			/*SECCIONES EN LAS QUE NO SE PERMITEN CAMBIOS*/
			if ((c_asigna == '300101') && (origen == 'm1')){
				document.getElementById('concup'+c_asigna).disabled = true;
				document.getElementById('sincup'+c_asigna).disabled = true;
			}
			if ( (c_asigna == '300113') && ((origen == '31') || (origen == '32') || (origen == '37')) ){
				document.getElementById('concup'+c_asigna).disabled = true;
				document.getElementById('sincup'+c_asigna).disabled = true;
			}
			/*FIN SECCIONES DONDE NO SE PERMITEN CAMBIOS*/
/// MEJORAR /// MEJORAR
